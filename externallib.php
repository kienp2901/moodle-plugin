...
require_once($CFG->dirroot . '/mod/quiznoems/lib.php');
 ...
 */
class mod_quiznoems_external extends external_api
{

    /**
     * Returns description of method parameters for create_quiznoems
     *
     * @return external_function_parameters
     */
    public static function create_quiznoems_parameters()
    {
     ...
     */
    public static function create_quiznoems(
        $courseid,
        $name,
        $intro = '',
        ...
        // Validate parameters
        $params = self::validate_parameters(self::create_quiznoems_parameters(), array(
            'courseid' => $courseid,
            'name' => $name,
            'intro' => $intro,
        ...
        // Check capabilities
        require_capability('mod/quiznoems:addinstance', $context);

        $module = $DB->get_record('modules', ['name' => 'quiznoems'], '*', MUST_EXIST);
        $moduleid = $module->id;

        // Prepare data for module creation
        $data = new stdClass();
        $data->modulename = 'quiznoems';
        ...
        // Create the quiznoems instance using add_moduleinfo
        $cm = add_moduleinfo($data, $course);

        if (!$cm) {
            throw new moodle_exception('errorcreatingquiznoems', 'mod_quiznoems');
        }

        // Get the created instance
        $quiznoems = $DB->get_record('quiznoems', array('id' => $cm->instance), '*', MUST_EXIST);

        return array(
            'id' => $quiznoems->id,
            'course' => $quiznoems->course,
            'name' => $quiznoems->name,
            'intro' => $quiznoems->intro,
            'introformat' => $quiznoems->introformat,
            'timecreated' => $quiznoems->timecreated,
            'timemodified' => $quiznoems->timemodified,
    ...
    /**
     * Returns description of method result value for create_quiznoems
     *
     * @return external_single_structure
     */
    public static function create_quiznoems_returns()
    {
    ...
    /**
     * Returns description of method parameters for update_quiznoems
     *
     * @return external_function_parameters
     */
    public static function update_quiznoems_parameters()
    {
        return new external_function_parameters([
            'cmid' => new external_value(PARAM_INT, 'Course module ID của quiznoems cần cập nhật'),
            'fields' => new external_multiple_structure(
                new external_single_structure([
                    'name' => new external_value(PARAM_TEXT, 'Tên quiznoems', VALUE_OPTIONAL),
                    'intro' => new external_value(PARAM_RAW, 'Mô tả quiznoems', VALUE_OPTIONAL),
     ...
     * @throws moodle_exception
     */
    public static function update_quiznoems($cmid, $fields)
    {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::update_quiznoems_parameters(), [
            'cmid' => $cmid,
            'fields' => $fields
        ]);

        // Get course module
        $cm = get_coursemodule_from_id('quiznoems', $params['cmid'], 0, false, MUST_EXIST);
        ...
        // Get the quiznoems instance
        $quiznoems = $DB->get_record('quiznoems', array('id' => $cm->instance), '*', MUST_EXIST);

        // Update fields provided
        foreach ($params['fields'] as $field_data) {
            foreach ($field_data as $field => $value) {
                if (isset($value) && $field !== 'availability' && property_exists($quiznoems, $field)) {
                    $quiznoems->{$field} = $value;
                }
            }
        }

        // Update quiznoems record
        $result = $DB->update_record('quiznoems', $quiznoems);

        if (!$result) {
            throw new moodle_exception('errorupdatingquiznoems', 'mod_quiznoems');
        }
        ...
        return [
            ...
            'quiznoemsid' => $quiznoems->id,
            'cmid' => $cmid
        ];
    }

    /**
     * Returns description of method result value for update_quiznoems
     *
     * @return external_single_structure
     */
    public static function update_quiznoems_returns()
    {
        return new external_single_structure([
            'status' => new external_value(PARAM_TEXT, 'Kết quả của thao tác'),
            'message' => new external_value(PARAM_TEXT, 'Thông báo kết quả'),
            'quiznoemsid' => new external_value(PARAM_INT, 'ID của quiznoems đã cập nhật'),
            'cmid' => new external_value(PARAM_INT, 'Course module ID của quiznoems')
        ]);
    }

    /**
     * Returns description of method parameters for get_quiznoems
     *
     * @return external_function_parameters
     */
    public static function get_quiznoems_parameters()
    {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
     ...
     * @throws moodle_exception
     */
    public static function get_quiznoems($id)
    {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::get_quiznoems_parameters(), array(
            'id' => $id,
        ));

        // Get the quiznoems instance
        $quiznoems = $DB->get_record('quiznoems', array('id' => $params['id']), '*', MUST_EXIST);
        $course = $DB->get_record('course', array('id' => $quiznoems->course), '*', MUST_EXIST);
        $context = \context_course::instance($course->id);

        // Check capabilities
        require_capability('mod/quiznoems:view', $context);

        // Get course module
        $cm = get_coursemodule_from_instance('quiznoems', $quiznoems->id, $course->id, false, MUST_EXIST);
    ...
    /**
     * Returns description of method result value for get_quiznoems
     *
     * @return external_single_structure
     */
    public static function get_quiznoems_returns()
    {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
    ...
    /**
     * Returns description of method parameters for delete_quiznoems
     *
     * @return external_function_parameters
     */
    public static function delete_quiznoems_parameters()
    {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
     ...
     * @throws moodle_exception
     */
    public static function delete_quiznoems($id)
    {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::delete_quiznoems_parameters(), array(
            'id' => $id,
        ));
        ...
        // Delete the quiznoems instance
        $result = quiznoems_delete_instance($params['id']);

        if (!$result) {
            throw new moodle_exception('errordeletingquiznoems', 'mod_quiznoems');
        }
    ...
    /**
     * Returns description of method result value for delete_quiznoems
     *
     * @return external_single_structure
     */
    public static function delete_quiznoems_returns()
    {
        return new external_single_structure(
            array(
