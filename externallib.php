 ...
 */
class mod_quizexercise_external extends external_api
{

    /**
     * Returns description of method parameters for create_quizexercise
     *
     * @return external_function_parameters
     */
    public static function create_quizexercise_parameters()
    {
     ...
     */
    public static function create_quizexercise(
        $courseid,
        $name,
        $intro = '',
        ...
        // Validate parameters
        $params = self::validate_parameters(self::create_quizexercise_parameters(), array(
            'courseid' => $courseid,
            'name' => $name,
            'intro' => $intro,
        ...
        // Check capabilities
        require_capability('mod/quizexercise:addinstance', $context);

        $module = $DB->get_record('modules', ['name' => 'quizexercise'], '*', MUST_EXIST);
        $moduleid = $module->id;

        // Prepare data for module creation
        $data = new stdClass();
        $data->modulename = 'quizexercise';
        ...
        // Create the quizexercise instance using add_moduleinfo
        $cm = add_moduleinfo($data, $course);

        if (!$cm) {
            throw new moodle_exception('errorcreatingquizexercise', 'mod_quizexercise');
        }
        ...
        return array(
            ...
            'intro' => $quizexercise->intro,
            'introformat' => $quizexercise->introformat,
            'timecreated' => $quizexercise->timecreated,
            'timemodified' => $quizexercise->timemodified,
    ...
    /**
     * Returns description of method result value for create_quizexercise
     *
     * @return external_single_structure
     */
    public static function create_quizexercise_returns()
    {
    ...
    /**
     * Returns description of method parameters for update_quizexercise
     *
     * @return external_function_parameters
     */
    public static function update_quizexercise_parameters()
    {
        return new external_function_parameters([
            'cmid' => new external_value(PARAM_INT, 'Course module ID của quizexercise cần cập nhật'),
            'fields' => new external_multiple_structure(
                new external_single_structure([
                    'name' => new external_value(PARAM_TEXT, 'Tên quizexercise', VALUE_OPTIONAL),
                    'intro' => new external_value(PARAM_RAW, 'Mô tả quizexercise', VALUE_OPTIONAL),
     ...
     * @throws moodle_exception
     */
    public static function update_quizexercise($cmid, $fields)
    {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::update_quizexercise_parameters(), [
            'cmid' => $cmid,
            'fields' => $fields
        ]);

        // Get course module
        $cm = get_coursemodule_from_id('quizexercise', $params['cmid'], 0, false, MUST_EXIST);
        ...
        // Get the quizexercise instance
        $quizexercise = $DB->get_record('quizexercise', array('id' => $cm->instance), '*', MUST_EXIST);

        // Update fields provided
        foreach ($params['fields'] as $field_data) {
            foreach ($field_data as $field => $value) {
                if (isset($value) && $field !== 'availability' && property_exists($quizexercise, $field)) {
                    $quizexercise->{$field} = $value;
                }
            }
        }

        // Update quizexercise record
        $result = $DB->update_record('quizexercise', $quizexercise);

        if (!$result) {
            throw new moodle_exception('errorupdatingquizexercise', 'mod_quizexercise');
        }
        ...
        return [
            ...
            'quizexerciseid' => $quizexercise->id,
            'cmid' => $cmid
        ];
    }

    /**
     * Returns description of method result value for update_quizexercise
     *
     * @return external_single_structure
     */
    public static function update_quizexercise_returns()
    {
        return new external_single_structure([
            'status' => new external_value(PARAM_TEXT, 'Kết quả của thao tác'),
            'message' => new external_value(PARAM_TEXT, 'Thông báo kết quả'),
            'quizexerciseid' => new external_value(PARAM_INT, 'ID của quizexercise đã cập nhật'),
            'cmid' => new external_value(PARAM_INT, 'Course module ID của quizexercise')
        ]);
    }

    /**
     * Returns description of method parameters for get_quizexercise
     *
     * @return external_function_parameters
     */
    public static function get_quizexercise_parameters()
    {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
     ...
     * @throws moodle_exception
     */
    public static function get_quizexercise($id)
    {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::get_quizexercise_parameters(), array(
            'id' => $id,
        ));

        // Get the quizexercise instance
        $quizexercise = $DB->get_record('quizexercise', array('id' => $params['id']), '*', MUST_EXIST);
        $course = $DB->get_record('course', array('id' => $quizexercise->course), '*', MUST_EXIST);
        $context = \context_course::instance($course->id);
        ...
        // Get course module
        $cm = get_coursemodule_from_instance('quizexercise', $quizexercise->id, $course->id, false, MUST_EXIST);
    ...
    /**
     * Returns description of method result value for get_quizexercise
     *
     * @return external_single_structure
     */
    public static function get_quizexercise_returns()
    {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
    ...
    /**
     * Returns description of method parameters for delete_quizexercise
     *
     * @return external_function_parameters
     */
    public static function delete_quizexercise_parameters()
    {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
     ...
     * @throws moodle_exception
     */
    public static function delete_quizexercise($id)
    {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::delete_quizexercise_parameters(), array(
            'id' => $id,
        ));
        ...
        // Delete the quizexercise instance
        $result = quizexercise_delete_instance($params['id']);

        if (!$result) {
            throw new moodle_exception('errordeletingquizexercise', 'mod_quizexercise');
        }
    ...
    /**
     * Returns description of method result value for delete_quizexercise
     *
     * @return external_single_structure
     */
    public static function delete_quizexercise_returns()
    {
        return new external_single_structure(
            array(
