 ...
 */
class mod_testexam_external extends external_api {

    /**
     * Returns description of method parameters for create_testexam
     *
     * @return external_function_parameters
     */
    public static function create_testexam_parameters() {
     ...
     */
    public static function create_testexam($courseid, $name, $intro = '', $introformat = FORMAT_HTML, 
        ...
        // Validate parameters
        $params = self::validate_parameters(self::create_testexam_parameters(), array(
            'courseid' => $courseid,
            'name' => $name,
            'intro' => $intro,
        ...
        // Check capabilities
        require_capability('mod/testexam:addinstance', $context);

        $module = $DB->get_record('modules', ['name' => 'testexam'], '*', MUST_EXIST);
        $moduleid = $module->id;

        // Prepare data for module creation
        $data = new stdClass();
        $data->modulename = 'testexam';
        ...
        // Create the testexam instance using add_moduleinfo
        $cm = add_moduleinfo($data, $course);

        if (!$cm) {
            throw new moodle_exception('errorcreatingtestexam', 'mod_testexam');
        }
        ...
        return array(
            'id' => $testexam->id,
            'course' => $testexam->course,
            'name' => $testexam->name,
            'intro' => $testexam->intro,
            'introformat' => $testexam->introformat,
            'timecreated' => $testexam->timecreated,
            'timemodified' => $testexam->timemodified,
    ...
    /**
     * Returns description of method result value for create_testexam
     *
     * @return external_single_structure
     */
    public static function create_testexam_returns() {
    ...
    /**
     * Returns description of method parameters for update_testexam
     *
     * @return external_function_parameters
     */
    public static function update_testexam_parameters() {
        return new external_function_parameters([
            'cmid' => new external_value(PARAM_INT, 'Course module ID của testexam cần cập nhật'),
            'fields' => new external_multiple_structure(
                new external_single_structure([
                    'name' => new external_value(PARAM_TEXT, 'Tên testexam', VALUE_OPTIONAL),
                    'intro' => new external_value(PARAM_RAW, 'Mô tả testexam', VALUE_OPTIONAL),
     ...
     * @throws moodle_exception
     */
    public static function update_testexam($cmid, $fields) {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::update_testexam_parameters(), [
            'cmid' => $cmid,
            'fields' => $fields
        ]);

        // Get course module
        $cm = get_coursemodule_from_id('testexam', $params['cmid'], 0, false, MUST_EXIST);
        ...
        // Get the testexam instance
        $testexam = $DB->get_record('testexam', array('id' => $cm->instance), '*', MUST_EXIST);

        // Update fields provided
        foreach ($params['fields'] as $field_data) {
            foreach ($field_data as $field => $value) {
                if (isset($value) && $field !== 'availability' && property_exists($testexam, $field)) {
                    $testexam->{$field} = $value;
                }
            }
        }

        // Update testexam record
        $result = $DB->update_record('testexam', $testexam);
        
        if (!$result) {
            throw new moodle_exception('errorupdatingtestexam', 'mod_testexam');
        }
    ...
    /**
     * Returns description of method result value for update_testexam
     *
     * @return external_single_structure
     */
    public static function update_testexam_returns() {
        return new external_single_structure([
            'status' => new external_value(PARAM_TEXT, 'Kết quả của thao tác'),
            'message' => new external_value(PARAM_TEXT, 'Thông báo kết quả'),
            'testexamid' => new external_value(PARAM_INT, 'ID của testexam đã cập nhật'),
            'cmid' => new external_value(PARAM_INT, 'Course module ID của testexam')
        ]);
    }

    /**
     * Returns description of method parameters for get_testexam
     *
     * @return external_function_parameters
     */
    public static function get_testexam_parameters() {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
            )
     ...
     * @throws moodle_exception
     */
    public static function get_testexam($id) {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::get_testexam_parameters(), array(
            'id' => $id,
        ));

        // Get the testexam instance
        $testexam = $DB->get_record('testexam', array('id' => $params['id']), '*', MUST_EXIST);
        $course = $DB->get_record('course', array('id' => $testexam->course), '*', MUST_EXIST);
        $context = \context_course::instance($course->id);
        ...
        // Get course module
        $cm = get_coursemodule_from_instance('testexam', $testexam->id, $course->id, false, MUST_EXIST);
    ...
    /**
     * Returns description of method result value for get_testexam
     *
     * @return external_single_structure
     */
    public static function get_testexam_returns() {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
                'course' => new external_value(PARAM_INT, 'Course ID'),
    ...
    /**
     * Returns description of method parameters for delete_testexam
     *
     * @return external_function_parameters
     */
    public static function delete_testexam_parameters() {
        return new external_function_parameters(
            array(
                'id' => new external_value(PARAM_INT, 'Test exam instance ID'),
            )
     ...
     * @throws moodle_exception
     */
    public static function delete_testexam($id) {
        global $DB;

        // Validate parameters
        $params = self::validate_parameters(self::delete_testexam_parameters(), array(
            'id' => $id,
        ));
        ...
        // Delete the testexam instance
        $result = testexam_delete_instance($params['id']);

        if (!$result) {
            throw new moodle_exception('errordeletingtestexam', 'mod_testexam');
        }
    ...
    /**
     * Returns description of method result value for delete_testexam
     *
     * @return external_single_structure
     */
    public static function delete_testexam_returns() {
        return new external_single_structure(
            array(
